/*
 * Copyright (c) 2002-2004 Todd C. Miller <Todd.Miller@courtesan.com>
 *
...
scan_atjobs(at_db *old_db, struct timeval *tv)
{
	...
	if (TEQUAL(old_db->mtim, statbuf.st_mtim)) {
	...
	new_db.mtim = statbuf.st_mtim;	/* stash at dir mtime */
...
atrun(at_db *db, double batch_maxload, TIME_T now)
{
	...
	for (batch = NULL, job = db->head; job; job = job->next) {
		...
		sprintf(atfile, "%s/%ld.%c", AT_DIR,
		    (long)job->run_time, job->queue);	/* safe */
	...
	if (batch != NULL
		...
		sprintf(atfile, "%s/%ld.%c", AT_DIR,
		    (long)batch->run_time, batch->queue);	/* safe */
...
run_job(atjob *job, char *atfile)
{
	...
	if ((statbuf.st_mode & 07777) != (S_IRUSR | S_IWUSR | S_IXUSR)) {
...
# endif /* BSD_AUTH */
	...
	if (nread != 0 || always_mail) {
		...
		if ((status = cron_pclose(mail)) != 0) {
			sprintf(buf, "mailed %lu byte%s of output"
			    " but got status 0x%04x\n", (unsigned long)bytes,
			    (bytes == 1) ? "" : "s", status); /* safe */
